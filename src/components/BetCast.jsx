...
// CSV export
const exportCSV = (data, fn = 'betcast_export.csv') => { const h = ['#', 'Εβδ', 'Στοίχ', 'Odds', 'Stake', 'Result', 'P/L', 'Budget']; const r = data.map((b) => [b.id, b.week, b.betNumber, safeNumber(b.odds).toFixed(2), safeNumber(b.stake).toFixed(2), b.result, safeNumber(b.profitLoss).toFixed(2), safeNumber(b.cumulativeBudget).toFixed(2)]); const csv = [h.join(','), ...r.map((x) => x.join(','))].join('\n'); const blob = new Blob(['\uFEFF' + csv], { type: 'text/csv;charset=utf-8;' }); const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = fn; a.click(); };
...
const TABLE_COLS = [
  ...
  { key: 'betNumber', label: 'Στ.', align: 'center' }, { key: 'odds', label: 'Odds', align: 'right' },
...
const BettingVisualizations = () => {
  ...
  // Data table
  const R_table = () => (<div className="card mb-section"><div className="flex-between" style={{ marginBottom: '0.75rem' }}><h3 className="card-chart-title" style={{ marginBottom: 0 }}>Πίνακας</h3><button className="export-btn" onClick={() => exportCSV(sortedTable)}>⬇ CSV</button></div><div className="data-table-wrap"><table className="data-table"><thead><tr>{TABLE_COLS.map((c) => <th key={c.key} style={{ textAlign: c.align }} onClick={() => handleSort(c.key)}>{c.label}<span className={`sort-arrow ${sortCol === c.key ? 'sort-arrow--active' : ''}`}>{sortCol === c.key ? (sortDir === 'asc' ? '▲' : '▼') : '↕'}</span></th>)}</tr></thead><tbody>{paged.length === 0 ? <tr><td colSpan={8} className="empty-state">Κανένα στοίχημα</td></tr> : paged.map((b) => (<tr key={b.id} className={highlightedWeek != null && b.week === highlightedWeek ? 'row-highlight' : ''}><td style={{ textAlign: 'center' }}>{b.id}</td><td style={{ textAlign: 'center' }}>{b.week}</td><td style={{ textAlign: 'center' }}>{b.betNumber}</td><td style={{ textAlign: 'right' }}>{safeNumber(b.odds).toFixed(2)}</td><td style={{ textAlign: 'right' }}>{safeNumber(b.stake).toFixed(2)}€</td><td style={{ textAlign: 'center' }} className={b.result === 'Win' ? 'cell-win' : 'cell-lose'}>{b.result === 'Win' ? '✓' : '✗'}</td><td style={{ textAlign: 'right' }} className={safeNumber(b.profitLoss) >= 0 ? 'cell-win' : 'cell-lose'}>{safeNumber(b.profitLoss) >= 0 ? '+' : ''}{safeNumber(b.profitLoss).toFixed(2)}€</td><td style={{ textAlign: 'right' }}>{safeNumber(b.cumulativeBudget).toFixed(2)}€</td></tr>))}</tbody></table></div>{totalPages > 1 && <div className="table-pagination"><button disabled={tablePage === 0} onClick={() => setTablePage((p) => p - 1)}>←</button><span>{tablePage + 1}/{totalPages}</span><button disabled={tablePage >= totalPages - 1} onClick={() => setTablePage((p) => p + 1)}>→</button></div>}</div>);
