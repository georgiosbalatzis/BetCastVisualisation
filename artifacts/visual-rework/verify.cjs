...
(async()=>{
  ...
  const cdp=await page.createCDPSession();const downloads=path.join(__dirname,'downloads');fs.mkdirSync(downloads,{recursive:true});await cdp.send('Page.setDownloadBehavior',{behavior:'allow',downloadPath:downloads});
  ...
  await page.screenshot({path:path.join(__dirname,'desktop-tooltip.png')});
  ...
  await page.evaluate(()=>sessionStorage.clear());await open('season=lastYear');await page.waitForSelector('.error-banner',{timeout:60000});await page.screenshot({path:path.join(__dirname,'retry-error.png'),fullPage:true});fail=false;await click('Επανάληψη');await page.waitForFunction(()=>!document.querySelector('.error-banner')&&Number(document.querySelector('.secondary-metrics dd')?.textContent)>0,{timeout:60000});result.flows.retry=true;
  ...
  fs.writeFileSync(path.join(__dirname,'verification-results.json'),JSON.stringify(result,null,2));console.log(JSON.stringify({chartChecks:result.charts.length,viewportChecks:result.viewports.length,flows:result.flows,errors:result.errors}));await browser.close();
